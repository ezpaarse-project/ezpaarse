                        ...
                        <div class="v-table__overflow">
                          <table class="v-datatable v-table theme--light">
                            <tbody v-if="report.general">
                              ...
                              <tr>
                                <td class="text-xs-left">{{ $t('ui.pages.process.report.ECsGenerated') }}</td>
                                <td class="text-xs-right">{{ report.general['nb-ecs'] }}</td>
                              </tr>
                              <tr>
                                <td class="text-xs-left">{{ $t('ui.pages.process.report.treatmentDuration') }}</td>
                                <td class="text-xs-right">{{ report.general['Job-Duration'] }}</td>
                              </tr>
                              <tr>
                                <td class="text-xs-left">{{ $t('ui.pages.process.report.logProcessingSpeed') }}</td>
                                <td class="text-xs-right">{{ report.general['process-speed'] }}</td>
                              </tr>
                              <tr>
                                <td class="text-xs-left">{{ $t('ui.pages.process.report.ECGenerationSpeed') }}</td>
                        ...
                        <div class="v-table__overflow">
                          <table class="v-datatable v-table theme--light">
                            <tbody v-if="report.stats">
                              <tr>
                                <td class="text-xs-left">{{ $t('ui.pages.process.report.recognizedPlatforms') }}</td>
                                <td class="text-xs-right">{{ report.stats['platforms'] }}</td>
                              </tr>
                              <tr>
                                <td class="text-xs-left">{{ $t('ui.pages.process.report.HTMLConsultations') }}</td>
                                <td class="text-xs-right">{{ report.stats['mime-HTML'] }}</td>
                              </tr>
                              <tr>
                                <td class="text-xs-left">{{ $t('ui.pages.process.report.PDFConsultations') }}</td>
                ...
                <v-card-text>
                  <v-layout row wrap>
                    <v-flex xs9 sm9 pr-2>
                        ...
                        <div class="v-table__overflow">
                          <table class="v-datatable v-table theme--light">
                            <tbody v-if="report.rejets">
                              <tr v-for="(reject, index) in report.rejets" :key="index" @mouseover="setCurrentReject(index)">
                      ...
                      <br />
                      <p v-if="report.general" v-html="$t('ui.pages.admin.jobs.relevantLogLinesRead', { relevantLogLines: (report.general['nb-lines-input'] - report.rejets['nb-lines-ignored']) })"></p>
                    </v-flex>

                    <v-flex xs3 sm3 pl-2 v-if="currentReject">
                      <h3 class="headline">{{ $t(`ui.pages.process.report.${currentReject}`) }}</h3>
                      <br />
                      <p v-html="$t(`ui.pages.process.report.descriptions.${currentReject}`)" class="text-xs-justify"></p>
              ...
              <v-card>
                <v-card-text>
                  <v-alert
                    ...
                    type="black"
                  >
                    <div v-for="(log, index) in logging" :key="index">{{ log.level }}: {{ log.message }}</div>
                  </v-alert>
                </v-card-text>
              </v-card>
            </v-expansion-panel-content>
