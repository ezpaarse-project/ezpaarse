...
A [skeleton](https://github.com/ezpaarse-project/ezpaarse-platforms/tree/master/js-parser-skeleton) can be used as a starting point. The directory structure and the files can be automatically generated by launching the `make init` command from the `platforms` directory (you may need to launch `make install` before).
