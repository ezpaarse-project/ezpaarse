...
Section $(menu+ezPAARSEmenu) SecMenuEZPAARSE
  ...
  !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
    ...
    CreateShortCut "$SMPROGRAMS\$StartMenuFolder\$(menu+8_traces)" "notepad.exe" "$INSTDIR\ezpaarselog.txt" 
    CreateShortCut "$SMPROGRAMS\$StartMenuFolder\$(menu+9_errors)" "notepad.exe" "$INSTDIR\ezpaarselogerror.txt" 
