...
var ridChecker = function () {
  ...
  */
  self.getISSN = function (rid) {
    ...
    result.rid = rid;
    if (issn.match(DoNotCheckPattern)) {
      result.checkable = false;
    } else if (issn.match(ISSNPattern)) {
      if (issn.length === 9) {
  ...
  */
  ...
  self.getISBN = function (rid) {
    ...
    result.rid = rid;
    if (rid.match(DoNotCheckPattern)) {
      result.checkable = false;
    } else if ((match = rid.match(ISBNPattern) !== null)) {
      if (isbn.length === 10) {
