    ...
    pkbManager.get(ec.platform, function (pkb) {
      if (pkb) {
        ...
        if (info) {
          ...
          self.job.logger.silly('The title_id couldn\'t be found in the "' + ec.platform + '" PKB [log line n°' + ec._meta.lineNumber + ']');
        ...
        self.job.logger.silly('No PKB found for platform "' + ec.platform + '" [log line n°' + ec._meta.lineNumber + ']');
      }
      if (self.job.geolocalize !== 'none') { geoloc(); }
      else { self.emit('ec', ec); }
    });
  } else {
    self.job.logger.silly('The parser couldn\'t find any PID in the given URL [log line n°' + ec._meta.lineNumber + ']');
    if (self.job.geolocalize !== 'none') { geoloc(); }
    else { self.emit('ec', ec); }
  }
};
