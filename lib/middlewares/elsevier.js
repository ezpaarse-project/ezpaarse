...
var cache  = require('../cache')('elsevier');
var elsevier = require('meta-els');
...
module.exports = function (req, res, job, saturate, drain) {
  ...
  job.report.set('general', 'elsevier-fails', 0);
  job.report.inc('general', 'elsevier-not-found', 0);
  job.report.set('general', 'elsevier-enriched-ecs', 0);

  if (!cache) {
    var err = new Error('failed to connect to mongodb, cache not available for elsevier');
    err.status = 500;
    return err;
  }


  var piiFields = elsevier.APIgetInfo(null, true);
    ...
    elsevier.resolve({'apiKey': EZapiKey, 'piis': piis}, function (err, list) {
      ...
      ecs.forEach(function (ec) {
        ...
        if (item) {
          ...
          job.report.inc('general', 'elsevier-enriched-ecs');
