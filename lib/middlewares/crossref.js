...
var fieldsPkbCross = {'publication_title' : 'doi-publication-title',
...
module.exports = function (req, res, job, saturate, drain) {
  ...
  for (let field in doiFields) {
    ...
    if (job.outputFields.removed.indexOf(field) === -1) {
      for (let element in fieldsPkbCross) {
        if (fieldsPkbCross[element] === field && enrichEnabledFields === 'true') {
          test = false;
        }
      }
      if (test) {
