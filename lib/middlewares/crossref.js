...
module.exports = function (req, res, job, saturate, drain) {
  ...
  var crossrefFields = (req.header('crossref-enrich-fields') || '').toLowerCase() == 'true';
  ...
  if (crossrefFields) {
    job.outputFields.added.push('crossref-publication_title');
    job.outputFields.added.push('crossref-publication_date');
    job.outputFields.added.push('crossref-publisher');
    job.outputFields.added.push('crossref-ISSN');
  }
  var buffer        = [];
  ...
  function aggregate(item, fieldsAggregate, ec) {
    if (item['type'] && /([a-z]+)\-([a-z]+)/.test(item['type'])) {
      ...
      ec[0]['crossref-publication_title'] = item['container-title'];
    } else {
      ec[0]['publication_title'] = ec[0]['publication_title'] || item['title'];
      ec[0]['crossref-publication_title'] = item['title'];
    }
    if (item['issued'] && item['issued']['date-parts'] && item['issued']['date-parts'][0]) {
      ...
      ec[0]['crossref-publication_date'] = item['issued']['date-parts'][0][0];
    }
    ...
    ec[0]['crossref-publisher'] = item['publisher'];
    ec[0]['type'] = item['type'];

    if (item['ISSN']) {
      ec[0]['crossref-ISSN'] = item['ISSN'];
