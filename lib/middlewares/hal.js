...
module.exports = function (req, res, job, saturate, drain) {
  ...
  function enhance(ec, next) {
    if (!activated || !ec || !ec.title_id || ec.platform !== 'hal' || notFound.has(ec.title_id)) {
      return next();
    }
    ...
    cache.findOne({ 'id': ec.title_id.toString() }, { data: 1 }, function (err, cachedDoc) {
      ...
      console.log(ec.title_id, cachedDoc)

      if (cachedDoc) {
        if (cachedDoc.data) {
          return release(ec.title_id, cachedDoc.data);
        } else if (cachedDoc.data === null) {
          notFound.add(ec.title_id);
          return release(ec.title_id, null);
        }
      }
