...
module.exports = function (req, res, job, saturate, drain) {
  ...
  function  pullBuffer() {
    ...
    istex.find(ec[0].unitid, function(err, result) {
      ...
      if (result.genre && result.genre[0] && result.genre[0] != null) {
      ...
      if (result.genre && result.genre[0]) { metaistex.istex_genre = result.genre[0]; }
  ...
  // funcontion enriche ec with data istex 
