...
module.exports = function (req, res, job, saturate, drain) {
  ...
  function  pullBuffer() {
    ...
    istex.find(ec[0].unitid, function(err, result) {
      ...
      var editeur = result.corpusName;
      metaistex.publisher_name = editeur;
      if (editeur.toUpperCase() == 'EEBO' && editeur.toUpperCase() == 'ECCO') {
        result.mime = 'TIFF';
      }
      if (result.host) {
        if (result.host.isbn) {
      ...
      if (result.language && result.language[0]) { metaistex.language = result.language[0]; }
