...
module.exports = function (req, res, job, saturate, drain) {
  ...
  function pullBuffer() {
    ...
    if (issnPattern.test(ec.print_identifier)) {
      ...
      job.logger.silly('Sudoc : no print_identifier ', ec);
    ...
    sudoc[sudoc2method](ec.print_identifier , function (err, doc) {
      if (err) {
        if (++nbErrors > 5) { 
          job.logger.verbose('Sudoc : to many errors, inactivating... ', err);
          activated = false;
        }
        release(ec.print_identifier, null);
  ...
  function release(print_identifier, data) {
    pending.get(print_identifier).forEach(function(ec) {
      ...
      if (data && data.query && data.query.resultNoHolding && data.query.resultNoHolding.ppn) {
