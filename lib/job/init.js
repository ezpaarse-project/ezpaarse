...
module.exports = function* init(req, res, options) {
  ...
  this.filterStatus     = /^true$/i.test(req.header('ezpaarse-filter-status') || 'true');
  ...
  const mwNames  = config.EZPAARSE_MIDDLEWARES.slice();
  const mwHeader = req.header('ezpaarse-middlewares');

  if (mwHeader) {
    const match = /^\(\s*(before|after)\s*(.+?)\s*\)(.+)$/i.exec(mwHeader.trim()) || [];

    let mwList      = match[3] || mwHeader;
    let insertPoint = mwNames.indexOf(match[2] || 'anonymizer');
