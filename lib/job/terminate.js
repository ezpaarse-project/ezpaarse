...
module.exports = function* terminateResponse(req, res) {
  ...
  if (!this.parsedLines && !res.headersSent) {
    res.set(this.statusHeader, 4003);
    res.set(this.msgHeader, statusCodes['4003']);
    res.writeHead(400);

    this.logger.warn(`The first ${this.maxParseAttempts} line(s) couldn't be parsed`);
    this.report.set('general', 'status', 4003);
    this.report.set('general', 'status-message', statusCodes['4003']);
  }
