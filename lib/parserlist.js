...
ParserList.prototype.clearCachedParsers = function () {
  ...
  for (var p in this.parsers) {
    ...
    var mod  = require.resolve(file);

    if (!require.cache[mod]) { continue; }
...
ParserList.prototype.init = function (callback) {
  ...
  fs.readdir(platformsDir, function (err, items) {
    ...
    (function nextItem() {
      var item = items.pop();
      if (!item) { return callback(errors, duplicates); }
      ...
      fs.stat(dir, function (err, stat) {
        ...
        if (!stat.isDirectory()) { return nextItem(); }
...
ParserList.prototype.addDomainsOf = function (platform, callback) {
  ...
  fs.readFile(manifestFile, function (err, json) {
    ...
    var parser          = pp.getParser(manifest.name);
    var isNode          = parser.language === 'node';
    var domainsPkbField = manifest['pkb-domains'];

    if (!parser || parser.language === '') {
      return callback(new Error('No valid parser found for ' + platform));
    }
