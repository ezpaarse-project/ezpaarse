...
var Job = function (req, res, jobID, options) {
  ...
  function terminateResponse() {
    ...
    if (self.notifications.mail.length && config.EZPAARSE_ADMIN_MAIL) {
      var reportUrl = req.ezBaseURL + '/report/' + self.jobID;
      var html = '';
      html += '<h2>Your job is completed.</h2>';
      html += '<p>ID: ' + self.jobID;
      html += '<br>You can get the report in this mail or ';
      html += util.format('<a href="%s">view it online</a>.</p>', reportUrl);

      html += '<h3>Alerts :</h3>';
      if (self.alerts.length) {
        html += '<ul><li>' + self.alerts.join('</li><li>') + '</li></ul>';
      } else {
        html += '<p>No alert generated.</p>';
      }

      html += '<h3>Reject files :</h3>';
      html += '<ul>';
      self.rejects.forEach(function (reject) {
        var rejectUrl = req.ezBaseURL + '/' + self.jobID + '/lines-' + reject + '.log';
        var nbLines   = self.report.get('rejets', 'nb-lines-' + reject) || 0;
        html += util.format('<li><a href="%s">%s</a> (%d)</li>', rejectUrl, reject, nbLines);
      });
      html += '</ul>';

      var mail = mailer.mail();
      mail.subject('[ezPAARSE] Your job is completed')
          .html(html)
