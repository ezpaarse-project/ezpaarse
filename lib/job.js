...
var Job = function (req, res, jobID, options) {
  ...
  function init() {
    ...
    initializer.init(req, res, function (err) {
      ...
      // Add URL to denied ECs, we can't do it before because we need the extention
      self.report.set('general', 'URL-denied-ecs', logRoute + '/denied-ecs.' + self.fileExtension);
  ...
  var writeDeniedEvent = function (ec) {
    ...
    self.report.inc('general', 'nb-ec-denied');
  };
  ...
  function terminateResponse() {
    ...
    finalizeReport(function () {
      if (self.deniedStream) { self.deniedStream.end(); }
      if (self.ecsStream) {
  ...
  function start() {
    ...
    var partHandlingFunction = function (data, callback) {
      ...
      var pipeInputStreamToLazy = function (inputStream, part, cbEndRead) {
        ...
        self.deniedWriter.on('saturated', function () {
          pressureMaster.addPressure('deniedWriter');
        });
        self.deniedWriter.on('drain', function () {
          pressureMaster.removePressure('deniedWriter');
        });
