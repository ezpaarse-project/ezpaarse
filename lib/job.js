...
var Job = function (req, res, jobID, options) {
    ...
    fs.readdir(path.join(__dirname, 'init'), function (err, files) {
      ...
      initializer.use(function (req, res, job, next) {
        ...
        (function loadNext() {
          ...
          var saturate = function () { addPressure(mwName); };
          var drain = function () { removePressure(mwName); };
      ...
      initializer.use(function errorHandler(err, req, res, job, next) {
        ...
        if (self.socket) { self.socket.emit('joberror', err.code, msg); }
  ...
  function start() {
    ...
    splitter.on('data', function (line) {
      ...
      if (self.resIsDeferred && needHeartbeat) {
        res.write('.');
        needHeartbeat = false;
