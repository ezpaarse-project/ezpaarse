...
var Job = function (req, res, jobID, options) {
  ...
  function terminateResponse() {
    ...
    // Looks for alerts when the report is finalized
    ...
    // If requested, send an email with alerts
    if (self.alerts.length && config.EZPAARSE_ADMIN_MAIL && self.notifications.mail.length) {
      var text = 'The job ' + self.jobID + ' generated the following alerts :';
      text += '\n- ' + self.alerts.join('\n- ');

      var mail = mailer.mail();
      mail.subject('[ezPAARSE] Alerts')
          .message(text)
          .from(config.EZPAARSE_ADMIN_MAIL)
          .to(self.notifications.mail.join(','))
          .send(function (err) {
            self.report.set('alerts', 'mail-status', err ? 'fail' : 'success');
            finalize();
          });
    } else {
      finalize();
