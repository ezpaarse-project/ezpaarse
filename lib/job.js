...
var Job = function (req, res, jobID, options) {
  ...
  function init() {
    ...
    self.counterReporter = new CounterReporter();

    (req.header('COUNTER-Reports') || '').split(',').forEach(function (report) {
      report = report.trim().toLowerCase();
      var added = self.counterReporter.add(report);
      if (added) {
        baseReport.stats['url-counter-' + report] = logRoute + '/' + report + '.xml';
      }
    });
  ...
  function terminateResponse() {
    ...
    var reports = self.counterReporter.getReports();
    for (var type in reports) {
      fs.writeFileSync(path.join(self.jobPath, type + '.xml'), reports[type]);
    }
