...
function Job(req, res, jobID, options) {
  ...
  this._run = function () {
    co(function* () {
      yield treatment.insert(req.user ? req.user.username : 'Anonyme', jobID);
  ...
  function closeJob() {
    ...
    const report = self.report || null;
    if (report && report.report.general) {
      if (report.report.general['status']) {
        return treatment.update(self.jobID, 'error');
      }

      if (report.report.general['Job-Done']  === false & self.onAbort) {
        return treatment.update(self.jobID, 'abort');
      }
    }
    return treatment.update(self.jobID, 'completed');
  }
  ...
  function closeStreams() {
    return co(function* () {
      ...
      self.logger.end();
      yield Promise.all(self.logger.transports.map(transport => new Promise(res => {
        transport._stream.on('close', res);
