...
var predefSettings = require('../resources/predefined-settings.json');
...
var Job = function (req, res, jobID, options) {
  ...
  // Remove the job from memory when the response is closed
  ...
  function init() {
    ...
    fs.readdir(path.join(__dirname, 'init'), function (err, files) {
      if (err) { return closeStreams(function () { res.status(500).end(); }); }
      ...
      initializer.use(function (req, res, job, next) {
        ...
        (function loadNext() {
          ...
          var saturate = function () {
            ...
            if (job.saturate) { job.saturate(); }
          };
          var drain = function () {
            job.saturated.delete(mwName);
            if (job.saturated.size === 0 && job.drain) { job.drain(); }
          };
  ...
  function start() {
    ...
    // function used to handle a log file (a part)
    var partHandlingFunction = function (data, callback) {
      ...
      var pipeInputStream = function (inputStream, part, cbEndRead) {
        ...
        self.saturate = function () {
          pressureMaster.addPressure('job');
        };
        self.drain = function () {
          pressureMaster.removePressure('job');
        };

        pressureMaster.on('data', function (data) {
          splitter.write(iconv.decode(data, self.inputCharset));
        });
      ...
      if (contentEncoding && contentEncoding != 'gzip') {
        ...
        self._stop();
        if (self.socket) { self.socket.emit('joberror', 4005, statusCodes['4005']); }
        callback();
        return;
      }
      ...
      if (isGzip) {
        ...
        unzip.on('error', function (err) {
          ...
          self._stop();
          if (self.socket) { self.socket.emit('joberror', 4002, statusCodes['4002']); }
          callback();
          return;
        });
        part.pipe(unzip);
        pipeInputStream(unzip, part, callback);

      } else {
        // Not gziped
        self.logger.info('Request upload: start reading an unencoded part [' +
                    part.filename + '] [' + contentType + ']');
        // PassThrough stream is a workaround because if
        // "part" is directly given to pipeInputStream
        // data are strangely not readed
        var PassThrough = require('stream').PassThrough;
        var passthrough = new PassThrough();
        part.pipe(passthrough);
        pipeInputStream(passthrough, part, callback);
      }
    };
    ...
    var uploadError = function (err, status) {
      ...
      if (!self.aborted) {
        ...
        if (!res.headersSent) {
          res.set(statusHeader, status);
          res.set(msgHeader, statusCodes[statusString]);
          res.status(400);
        }
        self.report.set('general', 'status', status);
        self.report.set('general', 'status-message', statusCodes[statusString]);
        self._stop();
        if (self.socket) { self.socket.emit('joberror', status, statusCodes[statusString]); }
      }
    };
    ...
    if (is(req, ['multipart/form-data'])) {
      ...
      partHandlingQueue.push({ part: req });
      // tells there is no more part to handle
      partHandlingQueue.push({ part: null });
    }
