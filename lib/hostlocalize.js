...
exports.resolve = function (host, cb, logger) {
  var r = {}, match;
  this.init(r);
  if ((match = /([^ ,]+)/.exec(host)) !== null) {
    host = match[0];
  } else {
    if (logger) { logger.warn('Error : host ', host, ' unrecognized pattern'); }
  ...
  dns.lookup(
    ...
    function (err, add, fam) {
      if (err) {
        if (logger) { logger.warn("Error : host ", host, " = ", JSON.stringify(err)); }
      ...
      if (geo === null) {
        if (logger) { logger.warn("Error : address lookup failed ", add); }
