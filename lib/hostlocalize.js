  ...
  if (type === 'dns-lookup') {
    dns.lookup(
      ...
      function (err, add, fam) {
        ...
        if (geo === null) {
          if (logger) { logger.silly("Error : dns address lookup failed " + add); }
          else { console.error("Error : dns address lookup failed ", add); }
          cb(r);
          return;
        }
        r = {
    ...
    if (geo === null) {
      if (logger) { logger.silly("Error : geoip address lookup failed ", host); }
      else { console.error("Error : geoip address lookup failed ", host); }
      cb(r);
      return false;
    }
    r = {
