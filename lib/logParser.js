...
module.exports = function (logFormat, dateFormat, replacer) {
  ...
  if (logFormat) {
    ...
    if (Object.keys(properties).length > 0) {
      that.preferedFormat             = {};
      that.preferedFormat.properties  = properties;
      that.preferedFormat.dateFormat  = dateFormat || 'DD/MMM/YYYY:HH:mm:ss Z';
      that.preferedFormat.replacer    = replacer || '-';
    }
  }
  ...
  that.parse = function (line) {
    ...
    if (that.preferedFormat)  { ec = that.extract(log, that.preferedFormat); }
    if (!ec && that.format)   { ec = that.extract(log, that.format); }
