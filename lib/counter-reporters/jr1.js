...
JR1Reporter.prototype.generateXML = function (periods) {
  ...
  self.journals.forEach(function (journal) {
    if (!journal.pissn && !journal.eissn) { return; }
...
JR1Reporter.prototype.generateCSV = function (periods) {
  var nbCol        = periods.length + 10;
  var reportHeader = '';
  var report       = '';

  var fillRow = function (currentCol) {
    var str = '';
    var n   = nbCol - currentCol;
  ...
  reportHeader += fillRow(2); //L1
  reportHeader += fillRow(1); //L2 - line for "customer", blank for now
  reportHeader += fillRow(1); //L3 - line for "institutional identifier", blank for now

  reportHeader += "Period covered by Report";
  reportHeader += fillRow(1); //L4
  reportHeader += moment(periods[0][0]).format("YYYY-MM-DD");
  reportHeader += " to ";
  reportHeader += moment(periods[periods.length - 1][1]).format("YYYY-MM-DD");
  reportHeader += fillRow(1); //L5

  reportHeader += "Date run";
  reportHeader += fillRow(1); //L6
  reportHeader += moment().format('YYYY-MM-DD');
  reportHeader += fillRow(1); //L7
  ...
  this.journals.forEach(function (journal) {
    ...
    report += journal.eisbn + ',' + journal.pisbn + '\n';
  });
