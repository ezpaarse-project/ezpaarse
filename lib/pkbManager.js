  ...
  that.pkbRequestQueue = async.queue(function (task, queueCallback) {
    ...
    if (pkb) {
      ...
      var pkbFile = that.pkbFolder + '/' + platform + '.pkb.csv';
        ...
        // var pkbFiles = fs.readdirSync(that.pkbFolder);
        // var pkbFiles  = pkbFolder.filter(function (file) {
        //   return regexp.test(file);
        // });
        // for (var i = 0, l = pkbFiles.length; i < l; i++) {
        //   var pkbFile = that.pkbFolder + '/' + pkbFiles[i];
