...
module.exports = function ecmaker() {
  ...
  maker.process = function (callback) {
    ...
    req.pipe(resultStream).on('finish', function () {
      ...
      request.get(reportURL, function (error, res, body) {
        if (error || !res) {
          return sendError(util.format("The report could not be downloaded");
        ...
        request(reportURL.replace(".json", ".html?standalone=1"))
