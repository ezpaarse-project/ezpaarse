...
module.exports = function (req, res, callback) {
  ...
  for (var i = 0, l = proxies.length; i < l; i++) {
    ...
    if (logFormat) {
      logProxy = proxy;
      job.report.set('general', 'input-log-type', proxy);
      job.report.set('general', 'input-log-format', logFormat);
      break;
    }
  }

  job.logParser = new LogParser(job.logger, logFormat, logProxy, dateFormat);
  callback(null);
};