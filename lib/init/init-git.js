...
module.exports = function (req, res, job, next) {
  ...
  git.branch(function (err, str) {
    job.report.set('general', 'git-branch', str ? str : 'N/A');

    git.tag(function (err, str) {
      if (str) { job.report.set('general', 'git-tag', str ? str : 'N/A'); }

      git.long(function (err, str) {
        if (str) { job.report.set('general', 'git-last-commit', str ? str : 'N/A'); }
