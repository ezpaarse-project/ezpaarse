...
module.exports = function (req, res, job, next) {
  ...
  function getWriterOutputStream(resType, ext) {
    ...
    job.ecsStream = stream = iconv.encodeStream(job.outputCharset || 'utf-8');
  ...
  function setDeniedWriter(type, ext) {
    var deniedPath = path.join(job.jobPath, '/denied-ecs.' + (ext || type));
  ...
  if (job.cleanOnly) {
    ...
    case 'application/jsonstream':
      ...
      job.headers['Content-Type'] = 'application/json';
      job.writer = new Writer(getWriterOutputStream('application/json', 'json'), 'jsonstream');
