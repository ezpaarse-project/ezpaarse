  ...
  if (logFormat) {
    ...
    if (this.logger) {
      if (this.extractor) {
        ...
        this.logger.info('Built regexp: ' + this.extractor.regexpLiteral);
      } else {
...
LogParser.prototype.getRegexp = function (literal) {
  if (!this.extractor) {
    return undefined;
  }
  return literal ? this.extractor.regexpLiteral : this.extractor.regexp;
};
