...
function LogParser(logger, logFormat, proxy, dateFormat, relativeDomain) {
  ...
  if (logFormat) {
    ...
    this.extractor = this.getExtractor(logFormat, proxy, dateFormat);
...
LogParser.prototype.getExtractor = function (logFormat, proxy, dateFormat) {
  var extractor;

  switch (proxy) {
  case 'ezproxy':
    extractor = require('./proxyformats/ezproxy.js')(logFormat);
    break;
  case 'apache':
    extractor = require('./proxyformats/apache.js')(logFormat);
    break;
  case 'squid':
    extractor = require('./proxyformats/squid.js')(logFormat);
    break;
  }
  if (extractor && extractor.regexp && extractor.properties) {
    extractor.dateFormat = dateFormat  || 'DD/MMM/YYYY:HH:mm:ss Z';
  } else {
    extractor = undefined;
  }
...
LogParser.prototype.autoDetect = function (line) {
  ...
  for (var proxy in formats) {
    ...
    for (var i = 0, l = proxyFormats.length; i < l; i++) {
      ...
      extractor = this.getExtractor(autoFormat.format, proxy, autoFormat.dateFormat);
      if (!extractor) { continue; }

      ec = this.extract(line, extractor);
      if (ec) {
        if (this.logger) { this.logger.info('Format found: ' + extractor.regexp); }
        extractor.dateFormat = extractor.dateFormat || 'DD/MMM/YYYY:HH:mm:ss Z';
        this.extractor = extractor;
        return ec;
      } else {
        if (this.logger) { this.logger.info('Format doesn\'t match: ' + extractor.regexp); }
      }
    }
  }
