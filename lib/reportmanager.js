...
ReportManager.prototype.updateGeneralUsage = function (callback) {
  ...
  for (var value in self.report.stats) {
    var usageVal  = usage.get('stats', value);
    var reportVal = self.get('stats', value);

    if (typeof reportVal !== 'number' || isNaN(reportVal)) { continue; }
    if (typeof usageVal !== 'number' || isNaN(usageVal)) {
      usage.set('stats', value, reportVal);
    } else {
      usage.set('stats', value, usageVal + reportVal);
    }
  }
