...
var PKBManager = function () {
  ...
  self.pkbRequestQueue = async.queue(function (task, queueCallback) {
    ...
    var pkb = self.knowledge[platform];
                  ...
                  if (exists) {
                    csvextractor.extract(missFile, ['pid'], function (missRecords) {
                      ...
                      for (var i in missRecords) {
                        pkb.get(missRecords[i].pid);
                      }
                      self.knowledge[platform] = pkb;
                      callback(pkb);
                      queueCallback();
                    }, {silent: true});
                  } else {
                    self.knowledge[platform] = new pkbGetter(platform, records);
                    callback(self.knowledge[platform]);
                    queueCallback();
                  }
                });
              }, {silent: true, key: 'pid'});
