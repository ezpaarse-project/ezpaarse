...
function PKBGetter(platform, pkb) {
  ...
  self.missQueue = async.queue(function (titleID, callback) {
    fs.exists(self.missingFile, function (exists) {
      if (!exists) {
        fs.writeFile(self.missingFile,
          'title_id\tprint_identifier\tonline_identifier\turl\n' + titleID + '\t\t\t', callback);
      } else {
            ...
            if (pkbFiles.length > 0) {
              csvextractor.extract(pkbFiles, [], function (err, records) {
                ...
                fs.exists(missFile, function (exists) {
                  if (exists) {
                    csvextractor.extract(missFile, ['title_id'], function (err, missRecords) {
                      if (err) {
              ...
              }, { silent: true, key: 'title_id', delimiter: '\t' });
