 ...
 */
fs.readdir(platformsFolder, function (err, files) {
  if (err) { throw err; }
        ...
        if (pfile && pfile.language !== '') {
          ...
          if (config.domains) {
            for (var j in config.domains) {
              var unique = parserlist.add(config.domains[j],
                pfile.path, isNode, config.name, config.longname);
              if (!unique) {
          ...
          if (domainsPkbField) {
            var pkbFolder = path.join(folderAddress, 'pkb');
                ...
                csvextractor.extract(pkbFiles, opts, function (err, records) {
                  ...
                  if (records.length > 0) {
                    records.forEach(function (record) {
                      parserlist.add(record[domainsPkbField],
                        pfile.path, isNode, config.name, config.longname);
                      if (!unique) {
