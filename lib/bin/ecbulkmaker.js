/*eslint no-sync: 0, max-nested-callbacks: [2,6]*/
'use strict';
...
exports.EcBulkMaker = function () {
  ...
  getLogFiles(sourceDir, function (err, files) {
    ...
    function processFiles(callback) {
      ...
      var resultFileGz = path.join(resDir, path.basename(file).replace(logReg, '.ec.csv.gz'));
      ...
      fs.exists(resultFile, function (exists) {
        ...
        fs.exists(resultFileGz, function (exists) {
          if (exists && !argv.force) {
            console.error('[Warning] %s exists, skipping', resultFileGz);
            return processFiles(callback);
          } // TODO: unlink html report if forcing
                ...
                if (err) {
                  ...
                  fs.unlink(koFile, function (err) {
                    if (err && err.code !== 'ENOENT') {
                      console.error('[Warning] The .ko file could not be deleted');
                    }
                    processFiles(callback);
                  });
                }
