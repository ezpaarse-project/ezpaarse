...
exports.patchVersionNumber = function () {
  ...
  var versionPattern = '([0-9]+\\.[0-9]+\\.[0-9]+|[0-9]+\\.[0-9]+\\.[0-9]+[0-9abcdef+]+)';
  var versionReg = new RegExp('^' + versionPattern + '$');
  ...
  c = c.replace(new RegExp('"' + versionPattern + '"'), '"' + version + '"');
  ...
  c = fs.readFileSync(f, 'utf8');
  c = c.replace(new RegExp('Version: ' + versionPattern), 'Version: ' + version);
  fs.writeFileSync(f, c, 'utf8');
  process.stdout.write(f + ' patched\n');
  ...
  c = fs.readFileSync(f, 'utf8');
  c = c.replace(new RegExp('APP_VERSION "' + versionPattern + '"'),
                'APP_VERSION "' + version + '"');
  fs.writeFileSync(f, c, 'utf8');
  process.stdout.write(f + ' patched\n');
