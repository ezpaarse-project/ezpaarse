...
exports.patchVersionNumber = function () {
  ...
  if (new RegExp('^' + version_pattern + '$').test(version)) {
    ...
    // doc/theme/_layout.html: ezPAARSE - Version: 0.0.3
    ...
    // views/footer.ejs: ezPAARSE - <%= __("Version") %>: 1.2.0
    f = __dirname + '/../../views/footer.ejs';
    c = fs.readFileSync(f, 'utf8');
    c = c.replace(new RegExp('ezPAARSE - <%= __\\("Version"\\) %>: ' + version_pattern),
                  'ezPAARSE - <%= __("Version") %>: ' + version);
    fs.writeFileSync(f, c, 'utf8');
    process.stdout.write(f + ' patched\n');
    ...
    c = fs.readFileSync(f, 'utf8');
    c = c.replace(new RegExp('APP_VERSION "' + version_pattern + '"'), 'APP_VERSION "' + version + '"');
    fs.writeFileSync(f, c, 'utf8');
    process.stdout.write(f + ' patched\n');
