...
function checkDependencies() {
  ...
  if (platform !== 'linux') {
    warningMessage('Cannot check dependencies on non-Linux platforms');
    return;
  }

  const pkgManagers = ['dpkg', 'rpm'];

  const pkgManager = pkgManagers.find(manager => {
    try {
      execSync(`command -v ${manager}`, { stdio: 'pipe' });
      return true;
    } catch (e) {
      return false;
    }
  ...
  if (!pkgManager) {
    errorMessage('Cannot determine the system package manager');
    return;
  }

  infoMessage(`Pakckage manager: ${pkgManager}`);
  ...
  dependencies.forEach((dep) => {
    try {
      if (pkgManager === 'dpkg') {
        execSync(`dpkg -l ${dep}`, { stdio: 'pipe' });
      ...
      if (pkgManager === 'dpkg') {
        infoMessage(`sudo apt-get install ${dep}`);
      } else if (pkgManager === 'rpm') {
        infoMessage(`sudo yum install ${dep}`);
        infoMessage(`sudo dnf install ${dep}`);
        infoMessage(`sudo zypper install ${dep}`);
      }
...
exports.checkConfig = async function () {
  ...
  const tests = [
    ...
    { title: 'System dependencies', fn: checkDependencies },
