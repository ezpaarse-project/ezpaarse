...
exports.ECMaker = function () {
  var optimist = require('optimist')
    ...
    .alias('year', 'y')
    .describe('year', 'If provided (YYYY), mask to choose a year' +
        ' (default current), none to skip period.')
    .alias('month', 'm')
    .describe('month', 'If provided (MM), mask to choose a month' +
        ' (default current), none to skip period.')
    .alias('day', 'd')
    .describe('day', 'If provided (DD), mask to choose a day' +
        ' (default all), none to skip period.')
    .alias('outpath', 'o')
      ...
      filtered.forEach(function (infile) {
        ...
        if (path.extname(infile) == ".log") {
          outfile = path.join((argv.outpath ? argv.outpath : ""),
            path.basename(infile).replace(".log", ".anonymized.log"));
          cmd = 'loganonymizer --input=' + infile + ' --output=';
          ...
          outfile = path.join((argv.outpath ? argv.outpath : ""),
            path.basename(infile).replace(".log.gz", ".anonymized.log"));
          cmd = 'zcat ' + infile + ' | loganonymizer --output=';
          console.log(cmd);
          if (shell.exec(cmd, {silent: true}).code !== 0) {
            console.error(infile + ' : not found');
          } else {
