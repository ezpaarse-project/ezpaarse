...
function initFieldSpliters(app, req, res, logger, callback) {
  ...
  for (var header in headers) {
    ...
    if (match && match[1] && match[2]) {
      num = match[1];
      if (!splitters[num])      { splitters[num] = {}; }
      if (!splitters[num].dest) { splitters[num].dest = []; }
      splitters[num].dest.push({ regexp: headers[header], fieldName: match[2] });
    } else {
      match = /^user-field([0-9]+)-(src|sep|residual)$/i.exec(header);
      if (match && match[1] && match[2]) {
        num = match[1];
        if (!splitters[num]) { splitters[num] = {}; }
        splitters[num][match[2]] = headers[header];
      }
    }
  }
  console.log(require('util').inspect(splitters, {depth: null}));
  callback(null, splitters);
}
