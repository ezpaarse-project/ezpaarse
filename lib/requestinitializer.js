 ...
 */
function initOutputFields(logger, req, callback) {
  logger.verbose('Initializing output fields');
  var fields       = req.header('OutputFields');
...
exports.init = function (logger, req, res, callback) {
  ...
  initAnonymization(logger, req, function (err, anonymize) {
    ...
    initOutputFields(logger, req, function (err, outputFields, fieldsUsage) {
      if (err) { callback(err); return; }
      ...
      initLogParser(logger, req, function (err, logParser) {
        ...
        initEncoding(logger, req, res, function (err, unzipReq, zipRes) {
          ...
          initWriter(logger, res, function (err, writer) {
            ...
            callback(null, unzipReq, zipRes, anonymize, logParser, writer, outputFields, fieldsUsage);
          });
        });
      });
    });
