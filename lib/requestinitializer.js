 ...
 */
function initDeduplication(app, req, res, logger, callback) {
  ...
  for (var header in headers) {
    ...
    if (regStrategy.test(header)) {
      ...
      options.removal = !/^false$/i.test(headers[header]);
    } else {
      ...
      if (match) {
        ...
        if (match) {
          options.fieldnames[match[1]] = headers[header];
        }
      }
    }
  }
    ...
    initFieldSplitters(app, req, res, logger, function (err, ufSplitters, userFields) {
      ...
      initOutputFields(app, req, res, logger, userFields, function (err, outputFields) {
        ...
        initLogParser(app, req, res, logger, function (err, logParser) {
          ...
          initDeduplication(app, req, res, logger, function (err, dedOptions) {
            if (err) { callback(err); return; }
            init.deduplication = dedOptions;
