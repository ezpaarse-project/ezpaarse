...
exports.isValid = function (ec, job) {
  ...
  var filterRedirs = job.hasOwnProperty('filterRedirs') ? job.filterRedirs : true;
  var filterStatus = job.hasOwnProperty('filterStatus') ? job.filterStatus : true;
  ...
  if (filterStatus) {
    ...
    } else if (ec.status && validCodes.indexOf(ec.status) === -1) {
      // Filters http codes other than 200 or 304
      return {
        valid: false,
        reason: `Ignored HTTP status code: ${ec.status} (only ${validCodes.join(',')} are kept)`
      };
    }
  }
