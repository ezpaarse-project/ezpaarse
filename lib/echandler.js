 ...
 */
var ECHandler = function (logger, sh, report) {
  var that = this;
  ...
  that.merge = function (log, result, pkb, parser) {
    ...
    if (ec.pid) {
      if (pkb) {
        ...
        if (id) {
          ...
          report.inc('nb-lines-pkb-miss-ecs');
        }
      } else {
    ...
    if (ec.issn || ec.eissn || ec.doi || ec.rtype || ec.mime) {
      ...
      if (!report.get('platform-' + ec.platform)) { report.inc('platforms'); }
      report.inc('platform-' + ec.platform);
      report.inc('rtype-' + ec.rtype);
      report.inc('mime-' + ec.mime);
    } else {
      logger.silly('Unqualified EC, not written');
      sh.write('unqualifiedECs', log.line + '\n');
      report.inc('nb-lines-unqualified-ecs');
    }
  }
