...
var debug        = require('debug')('reaper');
...
var FolderReaper = function (options) {
  ...
  that.lifetime  = options.lifetime  || { month: 1 };
  ...
  that.updateLimitDate = function () {
    ...
    limitDate.setFullYear(limitDate.getFullYear() - (that.lifetime.year || 0));
    limitDate.setMonth(limitDate.getMonth() - (that.lifetime.month || 0));
    ...
    limitDate.setSeconds(limitDate.getSeconds() - (that.lifetime.seconds || 0));
    limitDate.setMilliseconds(limitDate.getMilliseconds() - (that.lifetime.milliseconds || 0));
